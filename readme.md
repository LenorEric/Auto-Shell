...
After that, the user can choose to execute the command `[y]`, edit the command`[e]`, skip the current command without executing it`[s]`, ask questions about the current command`[a]`, or skip the current command and provide additional requirements to generate a new command`[i]`, quit current conversation`[q]`.
